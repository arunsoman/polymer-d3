Polymer({
    ...
    draw: function() {
      ...
      var yHeaders = [];
      me.getInputsProperty('y').forEach(function (ele) { 
        me.external.forEach (function (anExternal) { 
          if(anExternal.value === ele) { 
            yHeaders.push([anExternal.key, anExternal.value]);
          } 
        });
      });

      //load this from input
      var xInput = me.getInputsProperty('x'); 
      var xHeader = [me.external[xInput].key, me.external[xInput].value];
